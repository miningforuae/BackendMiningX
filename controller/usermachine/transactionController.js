...
export const checkPurchaseEligibility = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error checking purchase eligibility', error: error.message });
  }
};
...
export const sellUserMachine = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Error processing machine sale', error: error.message });
  } finally {
    session.endSession();
  }
};
...
export const getSaleHistory = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Error retrieving sale history', error: error.message });
  }
};